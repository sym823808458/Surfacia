...
- **SPES Overlay Support**: Switch the external overlay between the raw test set and the SPES candidate-prioritization layer
...
.. option:: --spes-csv PATH

   Optional ``SPES_Test_Set_Detailed`` CSV generated by the ML stage.

   When provided, the dashboard can switch the external overlay from the raw test set to the SPES ranking layer.
...
- **External Overlay Switch**: Toggle between no external points, raw test points, and the SPES layer
